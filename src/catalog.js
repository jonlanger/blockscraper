...
export const TERRAIN_GROUPS = [
  ...
  { id: 'grid', name: 'City streets & intersections', icon: 'car', blurb: 'Rework any street, grid or painted: one-ways, medians, bike lanes, roundabouts — or remove it' },
  { id: 'streets', name: 'Streets & paths', icon: 'route', blurb: 'Roads, bike paths and boulevards — paint over streets, paths, land and water' },
...
export const TERRAIN = Object.fromEntries([
  ...
  // City streets reconfigure a whole stretch between intersections, or a whole intersection (piece: 'seg' | 'x' | 'both').
  TR('avenue',     'grid', { name: 'Two-way Street',     icon: 'car',            cost: 4000, piece: 'both', desc: 'The standard street, with a lane each way and parking. Brings back a removed street or intersection.' }),
  TR('oneway',     'grid', { name: 'One-way Street',     icon: 'chevrons-right', cost: 3000, piece: 'seg', desc: 'Both lanes run the same way, with painted arrows. Apply again to reverse it.' }),
  TR('median',     'grid', { name: 'Planted Median',     icon: 'trees',          cost: 6000, piece: 'seg', scenic: true, desc: 'A raised median of trees and lamps down the middle, with bus lanes at the curb.' }),
  TR('bikeway',    'grid', { name: 'Protected Bike Lanes', icon: 'bike',         cost: 4000, piece: 'seg', desc: 'Green bike lanes behind flexible posts along both curbs. Buses move into the traffic lane.' }),
  TR('pedestrian', 'grid', { name: 'Pedestrian Street',  icon: 'footprints',     cost: 7000, piece: 'both', scenic: true, desc: 'Car-free paving curb to curb with trees, benches and lamps; traffic turns back. On an intersection it makes a plaza.' }),
  TR('nostreet',   'grid', { name: 'Remove Street',      icon: 'square-minus',   cost: 2000, piece: 'both', desc: 'Pave the stretch or intersection over as buildable ground. The tunnels below stay.' }),
  TR('signals',    'grid', { name: 'Traffic Signals',    icon: 'circle-dot',     cost: 3000, piece: 'x', desc: 'Signal poles on every corner and zebra crossings.' }),
  TR('stop',       'grid', { name: 'All-way Stop',       icon: 'ban',            cost: 1500, piece: 'x', desc: 'Stop signs and stop lines instead of signals.' }),
  TR('roundabout', 'grid', { name: 'Roundabout',         icon: 'orbit',          cost: 9000, piece: 'x', desc: 'A planted island that traffic circles around.' }),
  TR('scramble',   'grid', { name: 'Scramble Crossing',  icon: 'crosshair',      cost: 4000, piece: 'x', desc: 'Signals with diagonal crosswalks straight across the middle.' }),
  TR('xplaza',     'grid', { name: 'Intersection Plaza', icon: 'grid-2x2',       cost: 8000, piece: 'x', scenic: true, desc: 'Pave the whole crossing with a fountain and trees. Traffic turns back.' }),
  TR('road',      'streets', { name: 'Road',            icon: 'car',           cost: 2000, desc: 'A street like the city grid’s: curbs, sidewalks and lane lines sized to its width, with intersections where roads cross. Traffic drives in from city streets, and every City streets tool works on it — paint it 3 blocks wide for medians, bike lanes and roundabouts.' }),
  TR('bikelane',  'streets', { name: 'Bike Path',       icon: 'bike',          cost: 1600, desc: 'Green two-way cycle track with curbs and bollards. Cyclists ride it and out onto the streets.' }),
  TR('boulevard', 'streets', { name: 'Pedestrian Boulevard', icon: 'footprints', cost: 2200, scenic: true, desc: 'A car-free street of paving, trees, benches and lamps — a road set up as a Pedestrian Street, so the City streets tools can change it back.' }),
